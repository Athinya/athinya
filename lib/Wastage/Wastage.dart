...
class WastagePage extends StatefulWidget {
  const WastagePage({Key? key}) : super(key: key);
...
class _WastagePageState extends State<WastagePage> {
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text("Wastage Entry", style: HeadingStyle)
                                ],
                              ),
                              Row(
  ...
  Future<void> fetchWastageRecordNo() async {
    String? cusid = await SharedPrefs.getCusId();
    final response =
        await http.get(Uri.parse('$IpAddress/Wastage_serialno/$cusid/'));
  ...
  Future<void> postDataWithIncrementedSerialNo() async {
    ...
    try {
      ...
      var response = await http.post(
        Uri.parse('$IpAddress/Wastage_serialnoalldata/'),
          ...
          content: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'Kindly check your wastage detailss..!!',
  ...
  void _saveWastageDetailsAndRoundToAPI() async {
    String? qty = _qtyController.text;

    if (tableData.isEmpty || qty == null || _employeeController.text == "") {
      showEmptyWarning();
      return;
    }

    List<Map<String, dynamic>> WastageDetailsData = [];
    ...
    String WastageDetailsJson = json.encode(WastageDetailsData);

    String? cusid = await SharedPrefs.getCusId();
    String apiUrl = '$IpAddress/Wastage_Details_Roundalldata/';
    Map<String, dynamic> postData = {
      "cusid": cusid,
      'serialno': recordno,
      'date': _DateController.text,
      'agentname': _employeeController.text,
      'itemcount': tableData.length.toString(),
      'status': 'ManualStock',
      'WastageDetails': WastageDetailsJson,
    };
    ...
    try {
      ...
      if (mounted) {
        if (response.statusCode == 201) {
          ...
          await logreports(
              'Wastage Entry: ${_employeeController.text}_Insered');
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: () {
                                _saveWastageDetailsAndRoundToAPI();
                                tableData.clear();
                              },
                              style: ElevatedButton.styleFrom(
                                shape: RoundedRectangleBorder(
          ...
          child: SingleChildScrollView(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Wastage Details', style: HeadingStyle),
                  SizedBox(height: 20),
