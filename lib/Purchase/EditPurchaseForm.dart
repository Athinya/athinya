...
class _EditPurchaseEntryPageState extends State<EditPurchaseEntryPage> {
                                    ...
                                    children: [
                                      ...
                                      buildTextField(
                                        ...
                                        onFieldSubmittedCallback: (_) {
                                          print(
                                              "Moving focus from RecordNo to Date");

                                          // Move focus from RecordNo to Date
                                          _fieldFocusChange(
                                              context,
                                              RecordNooFocustNode,
                                              DateFocustNode);

                                          // Fetch details after focus change
                        ...
                        Container(
                          child: Column(
                            ...
                            children: [
                              Padding(
                                padding: EdgeInsets.only(
                                    left: Responsive.isDesktop(context) ? 5 : 8,
                                    top: 8),
                          ...
                          child: Column(
                            ...
                            children: [
                              Padding(
                                padding: EdgeInsets.only(
                                    left:
                                        Responsive.isDesktop(context) ? 5 : 25,
                                    top: 15),
                                child: Container(
                                  width: Responsive.isDesktop(context)
                          ...
                          child: Column(
                            ...
                            children: [
                              Padding(
                                padding: EdgeInsets.only(
                                    left:
                                        Responsive.isDesktop(context) ? 5 : 20,
                                    top: 8),
                                // child:
  ...
  Future<void> fetchSupplierContact() async {
    // Print the method entry point
    print("fetchSupplierContact called");
    ...
    String supplierName = SupplierNameController.text.trim(); // Ensure trimming
    print("Supplier name to match: $supplierName");

    bool contactFound = false;
    purchaseSuppliergstnoController.text =
        ''; // Reset the GST number before fetching
    try {
      String url = baseUrl;
      ...
      while (!contactFound) {
        ...
        if (response.statusCode == 200) {
          ...
          print("Data fetched: ${results.length} items");

          // Iterate through each supplier entry
          for (var entry in results) {
            // Print all details for each supplier entry
            print("Supplier Details:");
            print("ID: ${entry['id']}");
            print("CUS ID: ${entry['cusid']}");
            print("Name: ${entry['name']}");
            print("Address: ${entry['address']}");
            print("Contact: ${entry['contact']}");
            print("Balance: ${entry['balance']}");
            print("GST Number: ${entry['gstno']}");

            // Compare names case-insensitively after trimming
            if (entry['name'].toString().toLowerCase().trim() ==
              ...
              String gstNo =
                  entry['gstno'] ?? ''; // Use null coalescing to avoid null

              print("Contact found: $contactno");
              ...
              if (contactno.isNotEmpty) {
                ...
                // Check if the GST number is valid before setting it
                if (gstNo.isNotEmpty) {
                  purchaseSuppliergstnoController.text =
                      gstNo; // Set only if not empty
                } else {
                  purchaseSuppliergstnoController.text =
                      ''; // Clear if GST is empty
                  print(
                      "GST Number is empty. Please enter a valid GST number.");
                }
          ...
          if (!contactFound && data['next'] != null) {
            url = data['next'];
            print("Moving to the next page: $url");
          } else {
            // Exit the loop if no more pages or contact number found
            print("No more pages or contact found");
            break;
          }
        } else {
          throw Exception(
        ...
        children: [
          ...
          Container(
            ...
            child: Padding(
              ...
              child: ProductNameDropdown(), // Use the modified dropdown here
            ),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 0),
...
class _PurchaseDiscountFormState extends State<PurchaseDiscountForm> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> postDataToAPI(List<Map<String, dynamic>> tableData,
      ...
      // Get GST number and set default if blank
      // String gstNumber = widget.purchaseSuppliergstnoController.text.trim();
      // if (gstNumber.isEmpty) {
      //   gstNumber = ""; // Set a default value for GST number
      // }
      if (widget.purchaseSuppliergstnoController.text.isEmpty) {
        print("GST Number is empty. Please enter a valid GST number.");
        return; // Prevent the POST request if GST number is empty
      }
      String? cusid = await SharedPrefs.getCusId();
      Map<String, dynamic> postData = {
        "cusid": "$cusid",
        ...
        "agentid": widget.purchaseSupplierAgentidController.text,
        "contact": widget.purchaseContactController.text,
        "gstno": widget.purchaseSuppliergstnoController
            .text, // Ensure GST number is not blank
        "finaltaxable": finalTaxableController.text,
        "PurchaseDetails": productDetailsString,
      };

      print(
          "GST Number before post: ${widget.purchaseSuppliergstnoController.text}");
    ...
    //   }
    //   print("tableeeeeeee dataaaaaaaas  : $tableData");
    //   // String gstNumber = widget.purchaseSuppliergstnoController.text;
                                ...
                                child: Container(
                                  ...
                                  child: Container(
                                    ...
                                    child: TextFormField(
                                      ...
                                      onChanged: (newValue) {
                                        // Only update the discount percentage, but do not set the controller's text here
                                        // Let the user input the number and process it on form submission or on focus change
                                        calculateDiscountAmount();
                                        CalculateCGSTFinalAmount();
                                        CalculateSGSTFinalAmount();
                                        calculatetotalAmount();
                            ...
                            Padding(
                              ...
                              child: Container(
                                ...
                                child: ElevatedButton(
                                  focusNode: saveallButtonFocusNode,
                                  onPressed: () {
                                    if (widget.purchaseInvoiceNoController.text
                                            .isEmpty ||
                                        widget.purchaseSupplierAgentidController
                                            .text.isEmpty ||
                                        widget.tableData.isEmpty ||
                                        purchaseDisAMountController
                                            .text.isEmpty ||
                          ...
                          children: [
                            Padding(
                              padding: EdgeInsets.only(
                                  ...
                                  top: 0),
                              child: Container(
                                width: Responsive.isDesktop(context) ? 75 : 75,
                                child: ElevatedButton(
                                  onPressed: () {
                                    Clear();
                                  },
