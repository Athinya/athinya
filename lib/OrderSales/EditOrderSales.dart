...
class EditOrderSales extends StatefulWidget {
  const EditOrderSales({Key? key}) : super(key: key);

  @override
  State<EditOrderSales> createState() => _EditOrderSalesPageState();
}

class _EditOrderSalesPageState extends State<EditOrderSales> {
  int currentPage = 1;
  int pageSize = 10;
  ...
  Map<String, dynamic> mapOrderDetailsKeys(Map<String, dynamic> orderDetails) {
    return {
      'prodname': orderDetails['Itemname'],
      'rate': orderDetails['rate'],
      'qty': orderDetails['qty'],
      'cgst': orderDetails['cgst'],
      'sgst': orderDetails['sgst'],
      'amount': orderDetails['amount'],
      'taxable': orderDetails['retail'],
      'description': orderDetails['description'],
      'cgstPerc': orderDetails['cgstperc'],
      'sgstPerc': orderDetails['sgstperc'],
    };
  }
  ...
  Future<void> fetchOrderDetails() async {
    ...
    try {
      ...
      String url = '$IpAddress/EditOrderSales/$cusid/$billno/$date';
      ...
      if (response.statusCode == 200) {
        ...
        // Parse OrderDetails
        List<dynamic> orderDetailsList = json.decode(data['OrderDetails']);
        List<Map<String, dynamic>> mappedOrderDetailsList =
            orderDetailsList.map((item) {
          return mapOrderDetailsKeys(item);
        }).toList();
        setState(() {
          orderId = data['id'].toString();
          _CustomerController.text = data['cusname'].toString();
          _ContactController.text = data['contact'].toString();
          selectedStatus = data['ordertype'].toString();
          gstStatus = data['gststatus'].toString();
          _DisAmtController.text = data['discount'].toString();
          _DisPercController.text = data['dispperc'].toString();
          _SgstController.text = data['totsgst'].toString();
          _CgstController.text = data['totcgst'].toString();
          _TaxableAmtController.text = data['amount'].toString();
          _FinTaxController.text = data['finaltaxable'].toString();
          _AmountController.text = data['finalamount'].toString();
          _PaidAmountController.text = data['paidamount'].toString();
          _BalanceAmtController.text = data['balanceamount'].toString();
          _PayabaleController.text = data['payableamount'].toString();
          _DeliveryDateController.text = data['deliverydate'].toString();
          tableData = mappedOrderDetailsList;
          GstStatus = data['taxstatus'].toString();
          // print('Paytype: ${data['paytype']}');
          if (data['paytype'] is String) {
            _selectedPayType = data['paytype'].split(',');
          } else if (data['paytype'] is List) {
            _selectedPayType = List<String>.from(data['paytype']);
          }
        });
      } else {
        Warning();
        print('Failed to load order details. Status code');
        print('Response body: ');
      }
    } catch (e) {
...
// BillNo
  ...
  TextEditingController _billNoController = TextEditingController(text: 'OS');
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ElevatedButton(
            onPressed: () {
              SavetoSalesRoundAndDetails_tbl(orderId);
              _printResult();
            },
            style: ElevatedButton.styleFrom(
              shape: RoundedRectangleBorder(
...
// DateTimePicker
  ...
  FocusNode DeliveryDateFocus = FocusNode();
...
// PayType
  ...
  void _showPaytypeSelectDialogue() async {
    void _itemToggle(String itemValue) {
      ...
      FocusScope.of(context).requestFocus(DeliveryDateFocus);
    }
...
// Update To the Order Payment Table

  Future<void> UpdateOrderPayment(String FinBillNo) async {
    String? cusid = await SharedPrefs.getCusId();
    String apiUrl = '$IpAddress/OrderPaymentalldatas/';

    String? id = await fetchOrderPaymentIdByBillNo(FinBillNo);
    if (id == null) {
      print('Error: No OrderPaymentID found for billno $FinBillNo');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        print('Order Payemnt Data updated successfully');
        // print('Response body: ${response.body}');
      } else {
        print(
            'Failed to update OrderPaymentdata. Status code: ${response.statusCode}');
        // print('Response body: ${response.body}');
      }
    } catch (e) {
      print('Error: $e');
...
// Fetch To the Order Payment Id for Update

  Future<String?> fetchOrderPaymentIdByBillNo(String FinBillNo) async {
    String? cusid = await SharedPrefs.getCusId();
    String apiUrl = '$IpAddress/OrderPayment/$cusid/';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in data) {
          if (item['billno'] == FinBillNo) {
            return item['id'].toString();
          }
        }
      } else {
        print(
            'Failed to fetch OrderPaymentID for billno $FinBillNo. Status code: ${response.statusCode}');
        // print('Response body: ${response.body}');
      }
    } catch (e) {
      print('Error: $e');
...
// Update To Income Table

  Future<void> UpdateToIncome_tbl(String FinBillNo) async {
    ...
    Map<String, dynamic> putData = {
      "cusid": cusid,
      'dt': _DateController.text,
      'description': 'Order:$FinBillNo',
      'amount': Amount,
    };

    String? id = await fetchIncomeIdByBillNo(FinBillNo);
    if (id == null) {
      print('Error: No IncomeID found for billno $FinBillNo In Update');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(
            'Failed to update Income data. Status code: ${response.statusCode}');
      }
    } catch (e) {
      print('Error: $e');
    }
...
// Fetch To the Income Table Id for Update

  Future<String?> fetchIncomeIdByBillNo(String FinBillNo) async {
    int page = 1;
    bool hasMorePages = true;

    String? cusid = await SharedPrefs.getCusId();
    while (hasMorePages) {
      String apiUrl = '$IpAddress/IncomeEntryDetail/$cusid/?page=$page';
        ...
        if (response.statusCode == 200) {
          ...
          for (var item in data) {
            ...
            if (description.startsWith('Order:')) {
              String billNo = description.substring('Order:'.length).trim();
          ...
          hasMorePages = responseData['next'] != null;
          page++;
        } else {
          print(
              'Failed to fetch IncomeID for billno $FinBillNo. Status code: ${response.statusCode}');
          // print('Response body: ${response.body}');
          return null;
        }
      } catch (e) {
    ...
    print('Error: No IncomeID found for billno $FinBillNo');
    return null;
  }

// Save to Order details and Round Table

  void SavetoSalesRoundAndDetails_tbl(String Id) async {
    // print('Id: $Id');
    ...
    String apiUrl = '$IpAddress/OrderSalesRoundDetailsalldetails/$Id/';
    String billno = _billNoController.text;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print('Order Details Data Updated successfully');
        String billno = _billNoController.text;

        await logreports('OrderSalesBill: ${billno}_Updated');
        successfullyUpdateMessage(context);
        await UpdateOrderPayment(billno!);
        await UpdateToIncome_tbl(billno!);
        ClearFields();
      } else {
        print(
            'Failed to save Order details data. Status code: ${response.statusCode}');
