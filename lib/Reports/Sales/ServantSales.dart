...
void main() {
  runApp(ServantSalesReport());
}

class ServantSalesReport extends StatefulWidget {
  @override
  State<ServantSalesReport> createState() => _ServantSalesReportState();
}

class _ServantSalesReportState extends State<ServantSalesReport> {
  ...
  TextEditingController ServentNameController = TextEditingController();
  ...
  List<String> getDisplayedColumns() {
    return ['billno', 'dt', 'finalamount', 'servent'];
  }
  ...
  Future<void> fetchServentName() async {
    ...
    try {
      ...
      // print('All product categories: $ServentNameList');
    } catch (e) {
    ...
    return RawKeyboardListener(
      ...
      onKey: (RawKeyEvent event) {
        if (event is RawKeyDownEvent) {
          if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
            ...
            int currentIndex =
                ServentNameList.indexOf(ServentNameController.text);
            if (currentIndex < ServentNameList.length - 1) {
              setState(() {
                _selectedIndex = currentIndex + 1;
                ServentNameController.text = ServentNameList[currentIndex + 1];
      ...
      child: TypeAheadFormField<String>(
        textFieldConfiguration: TextFieldConfiguration(
          controller: ServentNameController,
        ...
        suggestionsCallback: (pattern) {
          if (_filterEnabled && pattern.isNotEmpty) {
            return ServentNameList.where(
                (item) => item.toLowerCase().contains(pattern.toLowerCase()));
          } else {
            return ServentNameList;
          }
        },
        itemBuilder: (context, suggestion) {
          final index = ServentNameList.indexOf(suggestion);
          return MouseRegion(
            onEnter: (_) => setState(() {
              _hoveredIndex = index;
            }),
        ...
        onSuggestionSelected: (String? suggestion) async {
          setState(() {
            ServentNameController.text = suggestion!;
  ...
  Future<void> fetchdatewisesales() async {
    ...
    if (response.statusCode == 200) {
      ...
      final filteredData = jsonData
          .where((data) => data['servent'] == ServentNameController.text)
          .toList();
      await logreports(
          "SalesServentReport: ${ServentNameController.text}_${foramtedletterstartdt} To ${foramtedletterenddt}_Viewd");
                        ...
                        children: [
                          ...
                          if (tableData.isNotEmpty)
                            ...tableData.map((data) {
                              ...
                              var serventname = data['servent'].toString();
  ...
  try {
    ...
    if (kIsWeb) {
      AnchorElement(
        ...
        ..setAttribute('download', 'ServentSales_Report ($formattedDate).xlsx')
        ..click();
    } else {
      final String path = (await getApplicationSupportDirectory()).path;
      final String fileName = Platform.isWindows
          ? '$path\\Excel ServentSales_Report ($formattedDate).xlsx'
          : '$path/Excel ServentSales_Report ($formattedDate).xlsx';
