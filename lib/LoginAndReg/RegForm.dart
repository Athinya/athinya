...
import 'package:ProductRestaurant/LoginAndReg/Login.dart';
...
class _RegistrationDialogState extends State<RegistrationDialog> {
                                            ...
                                            onPressed: () async {
                                              if (emailController
                                                ...
                                                if (emailExists) {
                                                  ...
                                                  // Just call the function directly without await
                                                  _launchChromeWithFlags();

                                                  // Show the dialog immediately after calling the function
                                                  showDialog(
  ...
  void _launchChromeWithFlags() async {
    if (Platform.isWindows) {
      ...
      await Process.run('google-chrome',
  ...
  Future<String?> fetchCsrfToken() async {
    final url = Uri.parse('$IpAddress/get-csrf-token/');
  ...
  Future<void> sendWhatsAppMessage(String name, String phoneNumber) async {
    ...
    if (csrfToken == null) {
      print('CSRF token is null. Cannot send WhatsApp message.');
      return;
    }
    ...
    try {
      final response = await http.post(
        Uri.parse('$IpAddress/send-whatsapp-message/'),
        headers: {
  ...
  void Register() async {
    if (nameController.text == "" ||
      ...
      if (response.statusCode == 201) {
        ...
        sendWhatsAppMessage(FullName, MobileNo);
        ...
        await insertPaymentMethod("PhonePe");
...
class _EmailOtpPageState extends State<EmailOtpPage> {
  ...
  Future<void> _sendEmailOtp() async {
    ...
    if (csrfToken == null) {
      _showWarningDialog('Failed to fetch CSRF token');
      return;
    }

    final url = Uri.parse('$IpAddress/send-email-otp/');
    final headers = {
      'Content-Type': 'application/json',
      'X-CSRFToken': csrfToken,
    };
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody['success']) {
          _showSuccessOTPSendDialog('OTP sent successfully to your email !!');
          setState(() {
            _generatedOtp = responseBody['otp'];
            _otpSent = true;
          });
        } else {
          _showWarningDialog('Failed to send OTP: ${responseBody['message']}');
        }
      } else {
