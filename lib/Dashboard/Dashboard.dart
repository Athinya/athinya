...
import 'package:ProductRestaurant/Modules/Chart.dart';
...
import 'package:ProductRestaurant/Payment/PlanSelectionPage.dart';
...
class _TopSellingCardviewState extends State<TopSellingCardview> {
  ...
  Future<void> fetchProductDetails() async {
    ...
    List<dynamic> allFetchedItems = []; // List to store items from all pages
    ...
    try {
      ...
      while (hasNextPage) {
        ...
        if (response.statusCode == 200) {
          ...
          if (fetchedItems is List) {
            allFetchedItems
                .addAll(fetchedItems); // Add fetched items to the list

            for (var item in fetchedItems) {
              String name = item['name'] ?? 'Unknown';
              String? base64Image = item['image']; // Get base64 image string

              Uint8List? imageBytes;
              if (base64Image != null && base64Image.isNotEmpty) {
                imageBytes = base64Decode(base64Image); // Decode base64 string
                // print('Fetched image for $name'); // Debug line
              } else {
                print('No image for $name'); // Debug line
              }

              for (var topSellingItem in topSellingItems) {
                if (topSellingItem['Itemname'] == name) {
                  productImages[name] = imageBytes; // Store the image bytes
                }
              }
            }
            setState(() {}); // Refresh UI
      ...
      // Optionally process or store allFetchedItems if needed
      // For example: allFetchedItems can be used for another purpose later on
  ...
  Uint8List? _getProductImage(int index) {
    String itemName = topSellingItems[index]['Itemname'];
    return productImages[itemName]; // Return the image bytes for the item name
  }
  ...
  Widget _buildCardWidget(int index) {
    ...
    // Retrieve image bytes for the current item
    Uint8List? itemImageBytes =
        itemName != 'Not Available' ? _getProductImage(index) : null;

    return SmallCard(
      name: itemName,
      imageBytes: itemImageBytes, // Pass the image bytes directly
    );
  }
}
...
class SmallCard extends StatelessWidget {
  final String name;
  final Uint8List? imageBytes; // Keep imageBytes nullable

  SmallCard({required this.name, this.imageBytes});
    ...
    return Padding(
      ...
      child: Card(
        ...
        child: Container(
          height: 35.0,
          padding: Responsive.isDesktop(context)
              ? EdgeInsets.only(top: 4.0, bottom: 4.0, left: 70, right: 4)
              : EdgeInsets.only(top: 4.0, bottom: 4.0, left: 10, right: 4),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
...
class _ShopInfoDialogState extends State<ShopInfoDialog> {
  ...
  Future<void> _updateShopDetails() async {
    ...
    try {
      // Check if shop ID exists for an update
      if (_shopId == null) {
        // Shop ID is null, perform a POST request
        final postResponse = await http.post(
          Uri.parse('$IpAddress/Shopinfo/'),
          headers: {'Content-Type': 'application/json'},
          body: json.encode(updatedData),
        );

        if (postResponse.statusCode == 201) {
          print('Shop details posted successfully');
          successfullySavedMessage(context);
          Navigator.of(context).pop(); // Close the dialog after saving
          _fetchShopDetails(); // Optionally refresh the details
        } else {
          print('Failed to post shop details: ${postResponse.body}');
          throw Exception(
              'Failed to post shop details. Status code: ${postResponse.statusCode}');
        }
      } else {
        // Shop ID exists, perform an update with PUT request
        ...
        if (putResponse.statusCode == 200) {
          ...
          print('Failed to update shop details: ${putResponse.body}');
          throw Exception(
              'Failed to update shop details. Status code: ${putResponse.statusCode}');
        }
      }
    } catch (e) {
      print('Error updating or posting shop details: $e');
    }
  }
